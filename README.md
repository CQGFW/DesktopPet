...
- 惯性抛掷：快速拖拽后松手，宠物沿拖动方向飞出，在当前屏幕内碰壁回弹、撞击时压扁，受阻尼与重力逐渐减速落回底部；飞行中左键抓取、右键菜单、滚轮或输入跟随都会立即接住它
- 始终置顶（默认开启，可在右键菜单切换）
- 自动走动（默认关闭，可在右键菜单开启）：宠物在当前屏幕右下 1/4 区域内随机游走，到达目标后停留数秒再走向下一处，向左走时自动转身；拖拽或菜单打开时暂停
- 呼吸起伏与头部跟随鼠标的动画；系统开启“减少动态效果”时自动停止动画（含自动走动、惯性抛掷与输入跟随）
...
| `desktoppet/fling.py` | 惯性抛掷的物理核心（释放速度采样、回弹积分，纯函数） |
| `desktoppet/autostart.py` | 开机自启动（注册表） |
| `desktoppet/animation_mixin.py` | 呼吸 / 头部跟随 / 互动动画 / 撞击形变 |
| `desktoppet/walk_mixin.py` | 自动走动状态机 |
| `desktoppet/fling_mixin.py` | 惯性抛掷与屏幕内回弹的窗口侧状态机 |
